...
def espirit(X, k, r, t, c, m=np.inf):
    """
    ...
    maps = np.zeros(np.append(np.shape(X), np.min([nc,m]))).astype(np.complex64)
    for idx in range(0, sx):
        for jdx in range(0, sy):
            for kdx in range(0, sz):
                ...
                u, s, _ = np.linalg.svd(Gq, full_matrices=False)

                # for ldx in range(0, nc):
                for ldx in range(0, np.min([nc,m])):
                    if (s[ldx]**2 > c):
